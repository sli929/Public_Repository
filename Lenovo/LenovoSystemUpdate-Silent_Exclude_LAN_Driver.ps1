...
-- ***** This version of the script EXCLUDES any LAN drivers ******
    * Purpose of this is to avoid any issues during OS provisioning phase with mecm, MDT, etc..
...
# First round:
   # Get ONLY updates that can be installed silently - if ".Installer.Unattended" property set to $True
    ...
    #>
    $updates = Get-LSUpdate | Where-Object {-not ($_.Title -match "LAN") -and ($_.Installer.Unattended) } -ErrorAction Continue # Retrieve all updates needed for the device that supports silent installation. (BIOS can be silent installation but requires a restart)
